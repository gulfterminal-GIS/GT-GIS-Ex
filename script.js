...
async function loadGeoJSON(content, filename) {
    ...
    if (geojsonData.features) {
      geojsonData.features.forEach(feature => {
        if (feature.geometry) {
          ...
          if (feature.geometry.type === 'Polyline' || feature.geometry.type === 'Line') {
  ...
  const warnings = [];
  console.warn = (msg) => {
    if (msg && msg.includes && msg.includes('fields types couldn\'t be inferred')) {
      warnings.push(msg);
    } else {
      originalWarn(msg);
    }
  ...
  try {
    ...
    console.log('Number of fields loaded:', layer.fields ? layer.fields.length : 0);
    ...
    const validFields = layer.fields ? layer.fields.filter(f => f.name !== 'OBJECTID').length : 0;
...
function formatAttributeValue(value, key) {
  ...
  const isDateField = dateKeywords.some(keyword => keyLower.includes(keyword));
  
  // Check if value looks like a timestamp (large number)
  const isTimestamp = typeof value === 'number' && value > 1000000000 && value < 10000000000000;
